...
export function renderObraCard(obra, { variant = 'list', className = '', distance = '', action = '', actionClass = '', featureId = obra?.featureId || obra?.id || '', showPhoto = true } = {}) {
  ...
  const importance = Number(obra?.importancia);
  const importanceLabel = Number.isFinite(importance) ? `Importancia ${4 - Math.min(3, Math.max(0, importance))} de 4` : '';
