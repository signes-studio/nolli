...
export function normalizeBuildingIdentity(building) {
    if (!building)
        return '';
...
export function esRolAdmin(role = state.userRole) {
    if (role === 'admin' || role === 'superadmin')
        return true;
    if (state.userRole === 'admin' || state.userRole === 'superadmin')
        return true;
    const email = String(state.userEmail || '').toLowerCase().trim();
    if (email === 'studio.signes@gmail.com' || email.includes('signes.studio') || email.includes('studio.signes'))
        return true;
    try {
        ...
        if (cached) {
            ...
            if (uEmail === 'studio.signes@gmail.com' || uEmail.includes('signes.studio') || uEmail.includes('studio.signes'))
                return true;
            if (u.role === 'admin' || u.role === 'superadmin')
                return true;
            if (u.app_metadata?.role === 'admin' || u.app_metadata?.role === 'superadmin')
                return true;
            if (u.user_metadata?.role === 'admin' || u.user_metadata?.role === 'superadmin')
                return true;
        }
        const cachedDb = localStorage.getItem('nolli_cached_db_profile');
        if (cachedDb) {
            const db = JSON.parse(cachedDb);
            if (db.role === 'admin' || db.role === 'superadmin')
                return true;
            const dbEmail = String(db.email || '').toLowerCase().trim();
            if (dbEmail === 'studio.signes@gmail.com' || dbEmail.includes('signes'))
                return true;
        }
    }
    catch { }
...
export function esRolEditor(role = state.userRole) {
    if (role === 'editor' || role === 'admin' || role === 'superadmin')
        return true;
    if (state.userRole === 'editor' || state.userRole === 'admin' || state.userRole === 'superadmin')
        return true;
    if (esRolAdmin(role))
        return true;
    try {
        ...
        if (cached) {
            const u = JSON.parse(cached);
            if (u.role === 'editor' || u.role === 'admin' || u.role === 'superadmin')
                return true;
            if (u.app_metadata?.role === 'editor' || u.app_metadata?.role === 'admin' || u.app_metadata?.role === 'superadmin')
                return true;
            if (u.user_metadata?.role === 'editor' || u.user_metadata?.role === 'admin' || u.user_metadata?.role === 'superadmin')
                return true;
        }
        const cachedDb = localStorage.getItem('nolli_cached_db_profile');
        if (cachedDb) {
            const db = JSON.parse(cachedDb);
            if (db.role === 'editor' || db.role === 'admin' || db.role === 'superadmin')
                return true;
        }
    }
    catch { }
...
export function normalizarCategoria(valor) {
    ...
    if (!categoria)
        return 'otro';
    if (categoria.includes('residencial') || categoria.includes('vivienda') || categoria.includes('casa'))
        return 'residencial';
    if (categoria.includes('religios') || categoria.includes('funer') || categoria.includes('tanatorio') || categoria.includes('cementerio'))
        return 'religioso_funerario';
    if (categoria.includes('industrial') || categoria.includes('logíst') || categoria.includes('logist'))
        return 'industrial_logistico';
    if (categoria.includes('comercial') || categoria.includes('comercio') || categoria.includes('mercado') || categoria.includes('hotel') || categoria.includes('oficina'))
        return 'comercial_terciario';
    if (categoria.includes('parque') || categoria.includes('plaza') || categoria.includes('paisaje') || categoria.includes('jard'))
        return 'espacio_publico_paisaje';
    if (categoria.includes('infraestruct') || categoria.includes('puente') || categoria.includes('estación') || categoria.includes('estacion') || categoria.includes('urban'))
        return 'infraestructura_urbanismo';
    if (categoria.includes('equipamiento') || categoria.includes('educativ') || categoria.includes('escuela') || categoria.includes('colegio') || categoria.includes('univers') || categoria.includes('biblioteca') || categoria.includes('museo') || categoria.includes('cultural') || categoria.includes('deport') || categoria.includes('salud') || categoria.includes('hospital') || categoria.includes('ayuntamiento'))
...
export function aplicarPreferenciasMapaColecciones(collections, userId) {
    ...
    return collections.map((col) => {
        ...
        const show_on_map = prefs[colId] !== undefined
            ? prefs[colId]
            : (col.show_on_map !== undefined ? col.show_on_map : true);
        return {
            ...col,
