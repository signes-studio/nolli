/* =========================================================================
   ITINERARIESCONFIG.TS — Configuración y rutas curatoriales de Nolli
   ========================================================================= */
...
export interface CuratedRoute {
  ...
  architectsFilter?: string[];
  architectFilter?: string;
  decadeFilter?: string | number;
  categoryFilter?: string;
  bboxFilter?: BoundingBoxFilter;
  keywords?: string[];
  work_ids?: Array<string | number>;
...
export function matchWorksForRoute(route: CuratedRoute, sourceList: Building[] = []): Building[] {
  ...
  if (route.addedByFilter) {
    const filterUpper = route.addedByFilter.toUpperCase();
    return list.filter((o) => {
      const addedBy = String(o.añadido_por || (o as unknown as { anadido_por?: string }).anadido_por || '').toUpperCase();
      return addedBy.includes(filterUpper);
    });
  }
  if (route.architectFilter) {
    const arqLower = route.architectFilter.toLowerCase();
    return list.filter((o) => {
      const arq = (Array.isArray(o.arquitectos) ? o.arquitectos.join(' ') : o.arquitectos || o.arquitecto || '').toLowerCase();
      return arq.includes(arqLower);
    });
  }
  ...
  if (route.categoryFilter) {
    const catLower = route.categoryFilter.toLowerCase();
    return list.filter((o) => String(o.categoria || '').toLowerCase() === catLower);
  }
  if (route.architectsFilter && route.architectsFilter.length) {
    const filters = route.architectsFilter;
    return list.filter((o) => {
      const arq = (Array.isArray(o.arquitectos) ? o.arquitectos.join(' ') : o.arquitectos || o.arquitecto || '').toLowerCase();
      return filters.some((name) => arq.includes(name.toLowerCase()));
    });
  }
  if (route.yearRange && route.yearRange.length === 2) {
    const [minYear, maxYear] = route.yearRange;
    return list.filter((o) => {
      const y = Number(o.año_construccion);
  ...
  if (route.keywords && route.keywords.length) {
    ...
    return list.filter((o) => {
      const arqs = Array.isArray(o.arquitectos) ? o.arquitectos.join(' ') : o.arquitectos || o.arquitecto || '';
      const text = `${o.nombre_obra} ${arqs} ${o.place || ''}`.toLowerCase();
