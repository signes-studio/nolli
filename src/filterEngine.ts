...
interface LngLatBoundsAcc {
  extend: (coord: [number, number]) => LngLatBoundsAcc;
}

declare const mapboxgl: {
  LngLatBounds: new (sw: [number, number], ne: [number, number]) => LngLatBoundsAcc;
} | undefined;

export interface FilterChip {
  id: string;
  type: 'category' | 'architect' | 'city' | 'itinerary' | 'search' | string;
  value?: string;
  label: string;
  color?: string;
  workIds?: Set<string>;
...
export function normalizarTexto(str: unknown): string {
  if (!str) return '';
  return String(str)
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
 ...
 */
export function obraCumpleFiltroIndividual(obra: Building | null | undefined, chip: FilterChip | null | undefined): boolean {
  ...
  if (chip.type === 'city' && chip.value) {
    ...
    const ciudad = normalizarTexto((obra as unknown as { ciudad?: string }).ciudad || obra.place || '');
    const pais = normalizarTexto((obra as unknown as { pais?: string }).pais || '');
  ...
  if (chip.type === 'search') {
    ...
    const haystack = normalizarTexto([
      ...
      (obra as unknown as { ciudad?: string }).ciudad,
      obra.place,
      (obra as unknown as { pais?: string }).pais,
      obra.categoria,
 ...
 */
export function obraCumpleFiltrosActivos(obra: Building): boolean {
  const chips = state.activeFilterChips as unknown as FilterChip[];
 ...
 */
export function obtenerObrasFiltradas(): Building[] {
 ...
 */
export function addFilterChip(chip: FilterChip): void {
  ...
  const existingIdx = chips.findIndex((c) =>
 ...
 */
export function removeFilterChip(chipId: string): void {
  if (!Array.isArray(state.activeFilterChips)) return;

  const chips = state.activeFilterChips as unknown as FilterChip[];
  state.activeFilterChips = chips.filter((c) => c.id !== chipId) as unknown as string[];
 ...
 */
export function removeLastFilterChip(): void {
 ...
 */
export function clearFilterChips(): void {
  state.activeFilterChips = [];
  restaurarDesdeFiltros();
}

function restaurarDesdeFiltros(): void {
  state.activeFilterChips = [];
 ...
 */
export function sincronizarFiltrosConMapa(): void {
  renderFilterChipsUI();
  actualizarFuenteMapa();
  document.dispatchEvent(new CustomEvent('radar:filters-changed'));
}
 ...
 */
export function renderFilterChipsUI(): void {
  ...
  const mapInstance = state.map as { flyTo?: (opts: unknown) => void; fitBounds?: (bounds: unknown, opts: unknown) => void } | null;
  if (count > 0 && mapInstance && typeof mapboxgl !== 'undefined') {
    const validCoords = filteredWorks
      ...
      .filter((coords): coords is [number, number] => coords != null && coords.length === 2 && Number.isFinite(coords[0]) && Number.isFinite(coords[1]));
    ...
    if (validCoords.length === 1 && validCoords[0]) {
      ...
      try {
        ...
        const bounds = validCoords.reduce<LngLatBoundsAcc>((b, c) => b.extend(c), initialBounds);
 ...
 */
export function initFilterEngine(): void {
  document.addEventListener('click', (e: MouseEvent) => {
    const target = e.target as HTMLElement | null;
    if (!target) return;

    // 1. Clic en la "X" individual de un chip
    const removeBtn = target.closest<HTMLElement>('[data-remove-chip-id]');
    if (removeBtn) {
      e.preventDefault();
      e.stopPropagation();
      const chipId = removeBtn.dataset.removeChipId;
      if (chipId) removeFilterChip(chipId);
      return;
    }

    // 2. Clic en "Deshacer último filtro" en el banner de estado vacío
    const undoBtn = target.closest<HTMLElement>('#btn-filter-undo');
    if (undoBtn) {
      e.preventDefault();
      e.stopPropagation();
