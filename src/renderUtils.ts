 ...
 */
export function renderInChunks(
...
export function createVirtualList<T>(
  container: HTMLElement | null,
  items: T[] = [],
  renderItem: ((item: T, index: number) => string) | null = null,
...
export function fetchWithCache<T>(
  cacheKey: string,
  fetchFn: () => Promise<T>,
  cache: Map<string, Promise<T> | T> = new Map()
): Promise<T> {
  if (cache.has(cacheKey)) {
    const cached = cache.get(cacheKey);
    if (cached && typeof (cached as Promise<T>).then === 'function') {
      return cached as Promise<T>;
    }
    return Promise.resolve(cached as T);
  }
  
  const promise = fetchFn().then((result: T) => {
...
export interface ShowNeoToastOptions {
  type?: string;
 ...
 */
export function showNeoToast(message: string, options: ShowNeoToastOptions = {}): void {
  const {
    actionText = null,
    onAction = null,
  ...
  let toast = document.getElementById('nolli-neo-toast') as ToastElement | null;
  if (!toast) {
  ...
  if (finalActionText) {
    const btn = toast.querySelector<HTMLButtonElement>('#nolli-toast-action-btn');
    if (btn) {
 ...
 */
export function initTabsScrollIndicator(
