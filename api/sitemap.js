...
const { slugify, extractCityName, isIgnoredArchitect, ARCHITECT_ALIASES, cleanArchitectName, ARCHITECT_SEPARATOR_REGEX } = require('./_lib/slugs.js');
...
async function fetchAllArchitectSlugs() {
  ...
  while (true) {
    ...
    for (const b of data) {
      if (b.arquitecto) {
        const names = b.arquitecto.split(ARCHITECT_SEPARATOR_REGEX).map((p) => cleanArchitectName(p)).filter(Boolean);
